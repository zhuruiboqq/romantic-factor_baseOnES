 ...
 * 用户权限的切面
 * <p/>
 * 1、当调用如下方法时，加缓存 {@link com.sishuok.es.sys.auth.service.UserAuthService#findRoles}
 ...
 * <p/>
 * 3。1、资源（Resource） 当修改资源时判断是否发生变化（如resourceIdentity，是否显示），如果变了清缓存 当删除资源时，清缓存 3.2、权限（Permission）
 * 当修改权限时判断是否发生变化（如permission，是否显示），如果变了清缓存 当删除权限时，清缓存
 * <p/>
 * 4、角色（Role） 当删除角色时，请缓存 当修改角色show/role/resourcePermissions关系时，清缓存
 * <p/>
 * 5.1、组织机构 当删除/修改show/parentId字段时，清缓存 5.2、工作职务 当删除/修改show/parentId字段时，清缓存
 * <p/>
 * 6。1、组 当修改组的默认组/show时，清缓存 当删除组时，清缓存
 * <p/>
 * 6.2、当删除组关系时 当新增/修改/删除的是某个用户的，只清理这个用户的缓存 其他清所有缓存
 * <p/>
 * 7、用户 修改时，如果组织机构/工作职务变了，仅需清自己的缓存
 * <p/>
 * 清理自己时也同时清理菜单的缓存
 * <p/>
 * TODO 1、异步失效缓存 2、预填充缓存（即把此刻失效的再通过异步任务查一次） 目前只查前100个吧 3、加二级缓存 提高失效再查的效率
 * <p/>
...
public class UserAuthCacheAspect extends BaseCacheAspect {
	...
	@Pointcut(value = "execution(* addGroupAuth(..)) " + "|| execution(* addOrganizationJobAuth(..)) " + "|| execution(* addOrganizationJobAuth(..))")
	private void authCacheEvictAllPointcut() {
	}

	@Pointcut(value = "(execution(* addUserAuth(*,..)) && args(arg, ..)) " + "|| (execution(* update(*)) && args(arg)) "
			+ "|| (execution(* save(*)) && args(arg)) " + "|| (execution(* delete(*)) && args(arg))", argNames = "arg")
	private void authCacheEvictAllOrSpecialPointcut(Object arg) {
	}

	/**
	 * 3.1、资源（Resource） 当修改资源时判断是否发生变化（如resourceIdentity，是否显示），如果变了清缓存 当删除资源时，清缓存
	 */
	...
	////清空整个缓存
	//////////////////////////////////////////////////////////////////////////////////
	@Before("(authServicePointcut() && authCacheEvictAllPointcut()) " + "|| (resourceServicePointcut() && resourceCacheEvictAllPointcut()) "
			+ "|| (permissionServicePointcut() && permissionCacheEvictAllPointcut()) " + "|| (roleServicePointcut() && roleCacheEvictAllPointcut()) "
			+ "|| (organizationServicePointcut() && organizationCacheEvictAllPointcut()) "
			+ "|| (jobServicePointcut() && jobCacheEvictAllPointcut()) " + "|| (groupServicePointcut() && groupCacheEvictAllPointcut()) "
	...
	public void authCacheClearSpecialOrAllAdvice(JoinPoint jp, Object arg) {
		...
		if (arg instanceof Auth) {//只清除某个用户的即可
			...
			if (arg instanceof Long) { //删除单个
				...
				log.debug("cacheName:{}, method:authCacheClearSpecialOrAllAdvice, evict user ids:{}", cacheName, Arrays.toString(userIds));
				evict(userIds);
			}
		}
	}
	...
	public void resourceMaybeCacheClearAllAdvice(Resource arg) {
		...
		if (!dbResource.getShow().equals(resource.getShow()) || (dbResource.getIdentity() == null && resource.getIdentity() != null)
				|| (dbResource.getIdentity() != null && !dbResource.getIdentity().equals(resource.getIdentity()))) {
	...
	public void permissionMaybeCacheClearAllAdvice(Permission arg) {
		...
		if (!dbPermission.getShow().equals(permission.getShow()) || !dbPermission.getPermission().equals(permission.getPermission())) {
	...
	public void roleMaybeCacheClearAllAdvice(Role arg) {
		...
		if (!dbRole.getShow().equals(role.getShow())
				...
				|| !(dbRole.getResourcePermissions().size() == role.getResourcePermissions().size() && dbRole.getResourcePermissions().containsAll(
	...
	public void organizationMaybeCacheClearAllAdvice(Organization arg) {
		...
		if (!dbOrganization.getShow().equals(organization.getShow()) || !dbOrganization.getParentId().equals(organization.getParentId())) {
	...
	public void jobMaybeCacheClearAllAdvice(Job arg) {
		...
		//只有当show/parentId发生改变时才清理缓存
		if (!dbJob.getShow().equals(job.getShow()) || !dbJob.getParentId().equals(job.getParentId())) {
	...
	public void groupRelationMaybeCacheClearAllOrSpecialAdvice(JoinPoint jp, Object arg) {
		...
		if (arg instanceof GroupRelation) {
			...
			log.debug("cacheName:{}, method:groupRelationMaybeCacheClearAllOrSpecialAdvice, evict user ids:{}", cacheName, Arrays.toString(userIds));
			evict(userIds);
		}
	}
	...
	public void userMaybeCacheClearSpecialAdvice(JoinPoint jp, Object arg) {
		...
		if ("update".equals(methodName)) {
			...
			if (!(user.getOrganizationJobs().size() == dbUser.getOrganizationJobs().size() && dbUser.getOrganizationJobs().containsAll(
					user.getOrganizationJobs()))) {

				log.debug("cacheName:{}, method:userMaybeCacheClearSpecialAdvice, evict user id:{}", cacheName, user.getId());
				evict(user.getId());
			}
