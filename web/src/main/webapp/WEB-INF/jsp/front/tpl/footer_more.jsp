<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<div class="footer_name_content fix">
	<div class="footer_top">
		<div class="line no_show left"></div>
		<img class="icon_left left" src="${ctx}/static/images/front/icon_left.png" />
		<div class="text left">
			<h1>
				<img src="${ctx}/static/images/front/foot_pic1.png" />
			</h1>
			<h2>罗 蔓 缘 素 婚 纱 摄 影</h2>
		</div>
		<div class="line no_show right"></div>
		<img class="icon_right right" src="${ctx}/static/images/front/icon_left.png" />
	</div>
	<div class="icons_link">
		<a href="JavaScript:;"><img src="${ctx}/static/images/front/xinlang_icon.png" /></a> <a href="JavaScript:;"><img src="${ctx}/static/images/front/weixin_icon.png" /></a> <a
			href="JavaScript:;"><img src="${ctx}/static/images/front/tengxun_icon.png" /></a> <a target="_blank" href="http://wpa.qq.com/msgrd?v=1&uin=707823233&site=qq&menu=yes"><img
			src="${ctx}/static/images/front/qq.png" /></a>
	</div>
	<p class="frist">© Romantic factor - 2015保留所有权利</p>
	<!-- <p>全国免费热线：400-888-8888     联系电话：0101-88988888</p>
        <p>地址：中国广州香港深圳上海杭州华南大厦10000号</p> -->
	<p>公司：广州市罗蔓缘素婚纱摄影有限公司</p>
</div>

<%@ include file="/WEB-INF/jsp/front/tpl/footer.jsp"%>