...
<div class="panel">
	...
	<form:form id="fileupload" method="post" commandName="m" cssClass="form-horizontal" enctype="multipart/form-data">
		...
		<form:hidden path="id" />
		<form:hidden id="workID" path="work.id" />

		<div class="control-group">
			<form:label path="artist.name" cssClass="control-label">艺术家</form:label>
			<div class="controls input-append">
				<form:input id="artistName" path="artist.name" cssClass="validate[required]" readonly="true" />
				<span class="add-on"><i class="icon-chevron-down"></i></span>
				<form:hidden id="artistId" path="artist.id" />
			</div>
		</div>
		...
		<div class="control-group">
			<div class="controls">
				<button type="button" class="btn show-upload">
					<i class="icon-eye-open"></i> 显示上传的文件名
				</button>
			</div>
		</div>
...
<div id="modal-gallery" class="modal modal-gallery hide fade" data-filter=":odd" tabindex="-1">
	...
	<div class="modal-footer">
		<a class="btn modal-download" target="_blank"> <i class="icon-download"></i> 下载
		</a> <a class="btn btn-success modal-play modal-slideshow" data-slideshow="5000"> <i class="icon-play icon-white"></i> 幻灯片显示
		</a> <a class="btn btn-info modal-prev"> <i class="icon-arrow-left icon-white"></i> 前一个
		</a> <a class="btn btn-primary modal-next"> 下一个 <i class="icon-arrow-right icon-white"></i>
		</a>
	</div>
</div>
<es:contentFooter />
...
<script id="template-download" type="text/x-tmpl">
    {% var workIDs = $.makeArray(); for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-download fade">
        ...
        <td class="error" colspan="2"><span class="label label-important">错误</span> {%=file.error%}</td>
        {% } else { %}
        <td class="name" style="width:80px">{%=file.id %} <input type='hidden' id='workID_{%=file.id %}' name='workID' value='{%=file.id %}'></td>
    ...
    {% workIDs.push(file.id); } $("#workID").val(workIDs.join(","));%}
</script>
...
<script type="text/javascript">
	$(function() {
		...
		$("[name='artist.name']").siblings(".add-on").andSelf().click(function() {
			$.app.modalDialog("参照", "${ctx}/basedata/artist/select/single;domId=artistId;domName=artistName", {
				width : 600,
				height : 450,
				callback : function() {
					$("[name='artist.name']").validationEngine('validate');
					return true;
				}
			});
		});
		...
		$('#fileupload').fileupload({
			url : '${ctx}/basedata/attachmentImage/ajaxUpload'
		});
